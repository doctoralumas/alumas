...
const modeDefs: Record<Mode, any[]> = {
  ...
  glucose: [
    { metric: "glucose", label: "Kan ÅŸekeri hedefi", unit: "mg/dL" }
  ],
  sleep: [
    { metric: "sleep-hours", label: "Uyku sÃ¼resi hedefi", unit: "saat" }
  ],
};
...
export default function HealthModule({ mode }: { mode: Mode }) {
  ...
  const title = mode === "blood-pressure" ? "Tansiyon & NabÄ±z" : mode === "glucose" ? "Kan Åekeri Takibi" : "Uyku DÃ¼zeni";
  const desc = mode === "blood-pressure" ? "Tansiyon Ã¶lÃ§Ã¼mlerinizi dÃ¼zenli takip edin." : mode === "glucose" ? "AÃ§lÄ±k ve tokluk kan ÅŸekeri Ã¶lÃ§Ã¼mleriniz." : "GÃ¼nlÃ¼k uyku sÃ¼resi ve kaliteniz.";
  ...
  function warningFor(r: any) {
    ...
    if (mode === "blood-pressure" && (outside("bp-systolic", r.systolic) || outside("bp-diastolic", r.diastolic))) return "Hedef aralÄ±ÄŸÄ± dÄ±ÅŸÄ±nda";
    if (mode === "glucose" && outside("glucose", glucoseMg(r))) return "Hedef aralÄ±ÄŸÄ± dÄ±ÅŸÄ±nda";
    if (mode === "sleep") { const h = (new Date(r.endedAt).getTime() - new Date(r.startedAt).getTime()) / 3600000; if (outside("sleep-hours", h)) return "Hedef sÃ¼renin dÄ±ÅŸÄ±nda"; }
    return null;
  }
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const form=e.currentTarget;const f=new FormData(form), body: any = Object.fromEntries(f.entries());
    ...
    <div className="page" style={{ maxWidth: "1200px" }}>
      ...
      <div style={{ marginBottom: "32px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div>
            <span className="kicker" style={{ color: color }}>GÃ¼nlÃ¼k Takip</span>
        ...
        <button onClick={() => setOpen(!open)} style={{ padding: "12px 24px", background: "#0f172a", color: "#fff", borderRadius: "100px", border: "none", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" }}>
          {open ? <X size={20} /> : <Plus size={20} />} {open ? "Kapat" : "Yeni KayÄ±t Ekle"}
        </button>
      </div>
        ...
        <form onSubmit={submit} style={{ background: "#fff", padding: "32px", borderRadius: "32px", border: "1px solid #e2e8f0", marginBottom: "32px", display: "flex", flexDirection: "column", gap: "24px", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.05)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px" }}>
            {mode === "blood-pressure" && (
              <>
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Sistolik (BÃ¼yÃ¼k)</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Diyastolik (KÃ¼Ã§Ã¼k)</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>NabÄ±z (BPM)</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Ã–lÃ§Ã¼m ZamanÄ±</label>
            ...
            {mode === "glucose" && (
              <>
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Ã–lÃ§Ã¼m DeÄŸeri</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Durum (AÃ§lÄ±k/Tokluk)</label>
                  <select name="context" style={{ width: "100%", padding: "14px 16px", borderRadius: "16px", border: "1px solid #cbd5e1", background: "#f8fafc", outline: "none" }}><option value="FASTING">AÃ§lÄ±k</option><option value="POSTPRANDIAL">Tokluk</option><option value="RANDOM">Rastgele</option><option value="BEDTIME">Gece</option></select>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Ã–lÃ§Ã¼m ZamanÄ±</label>
                  <input name="measuredAt" type="datetime-local" style={{ width: "100%", padding: "14px 16px", borderRadius: "16px", border: "1px solid #cbd5e1", background: "#f8fafc", outline: "none", color: "#475569" }} />
            ...
            {mode === "sleep" && (
              <>
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>Uyku BaÅŸlangÄ±cÄ±</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 600, color: "#475569", marginBottom: "8px" }}>UyanÄ±ÅŸ</label>
                ...
                <div>
                  ...
                  <select name="quality" style={{ width: "100%", padding: "14px 16px", borderRadius: "16px", border: "1px solid #cbd5e1", background: "#f8fafc", outline: "none" }}><option value="5">Ã‡ok Ä°yi (5)</option><option value="4">Ä°yi (4)</option><option value="3">Orta (3)</option><option value="2">ZayÄ±f (2)</option><option value="1">KÃ¶tÃ¼ (1)</option></select>
                </div>
              </>
            )}
          </div>
          <div>
            ...
            <input name="note" placeholder="Ã–lÃ§Ã¼mle ilgili notlarÄ±nÄ±z..." style={{ width: "100%", padding: "14px 16px", borderRadius: "16px", border: "1px solid #cbd5e1", background: "#f8fafc", outline: "none" }} />
          </div>
          <div style={{ display: "flex", gap: "12px", marginTop: "8px" }}>
            ...
            <button type="button" onClick={() => setOpen(false)} style={{ padding: "14px 32px", background: "#f1f5f9", color: "#475569", borderRadius: "16px", border: "none", fontWeight: 700, fontSize: "16px", cursor: "pointer" }}>Ä°ptal</button>
          </div>
      ...
      {latestWarning && (
        <div style={{ background: "#fef2f2", borderLeft: "4px solid #ef4444", padding: "16px 20px", borderRadius: "0 16px 16px 0", display: "flex", alignItems: "center", gap: "12px", marginBottom: "32px", color: "#b91c1c" }}>
          ...
          <div>
            ...
            <span style={{ fontSize: "13px" }}>{latestWarning}. Bu sistem bir tÄ±bbi tanÄ± aracÄ± deÄŸildir; endiÅŸeniz varsa saÄŸlÄ±k profesyoneline danÄ±ÅŸÄ±n.</span>
          </div>
        </div>
      )}

      {/* Hero Stats (Son Ã–lÃ§Ã¼m) */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "24px", marginBottom: "40px" }}>
        ...
        <div style={{ background: bg, padding: "32px", borderRadius: "32px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <span style={{ fontSize: "14px", fontWeight: 600, color: color, textTransform: "uppercase", letterSpacing: "1px", marginBottom: "12px" }}>Son Ã–lÃ§Ã¼m</span>
          
          {mode === "blood-pressure" ? (
            <>
              <strong style={{ fontSize: "42px", color: "#0f172a", lineHeight: 1, fontWeight: 800 }}>{latest ? `${latest.systolic}/${latest.diastolic}` : "--"}</strong>
              <small style={{ fontSize: "15px", color: "#475569", marginTop: "8px", fontWeight: 500 }}>mmHg {latest?.pulse ? `â€¢ NabÄ±z ${latest.pulse}` : ""}</small>
            </>
          ) : mode === "glucose" ? (
            <>
              <strong style={{ fontSize: "42px", color: "#0f172a", lineHeight: 1, fontWeight: 800 }}>{latest ? latest.value : "--"}</strong>
              <small style={{ fontSize: "15px", color: "#475569", marginTop: "8px", fontWeight: 500 }}>{latest?.unit || "mg/dL"} â€¢ {latest?.context === "FASTING" ? "AÃ§lÄ±k" : latest?.context === "POSTPRANDIAL" ? "Tokluk" : "Ã–lÃ§Ã¼m"}</small>
            </>
          ) : (
            <>
        ...
        <div style={{ background: "#f8fafc", padding: "32px", borderRadius: "32px", border: "1px solid #e2e8f0", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <span style={{ fontSize: "14px", fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "12px" }}>Toplam KayÄ±t</span>
          <strong style={{ fontSize: "42px", color: "#0f172a", lineHeight: 1, fontWeight: 800 }}>{rows.length}</strong>
          <small style={{ fontSize: "15px", color: "#94a3b8", marginTop: "8px", fontWeight: 500 }}>Sistemdeki genel geÃ§miÅŸiniz</small>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "20px", marginBottom: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: "14px", color: "#64748b" }}>SeÃ§ilen tarih aralÄ±ÄŸÄ±ndaki grafikleriniz.</p>
            </div>
          </div>
      ...
      <section style={{ background: "#fff", borderRadius: "32px", padding: "32px", border: "1px solid #e2e8f0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: "20px", color: "#0f172a" }}>GeÃ§miÅŸ Ã–lÃ§Ã¼mler</h2>
            <p style={{ margin: 0, fontSize: "14px", color: "#64748b" }}>TÃ¼m kayÄ±tlarÄ±nÄ±z ters kronolojik sÄ±rada.</p>
          </div>
        </div>
