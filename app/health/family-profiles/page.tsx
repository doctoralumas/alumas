    ...
    <div className="page" style={{ maxWidth: "800px" }}>
      <div style={{ marginBottom: "32px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <span className="kicker">Profil YÃ¶netimi</span>
            <h1 style={{ fontSize: "28px", color: "#0f172a", margin: "8px 0" }}>Gebelik & Ã‡ocuk Profilleri</h1>
            <p style={{ color: "#64748b", margin: 0, fontSize: "15px" }}>Gebelik takibi veya Ã§ocuÄŸunuz iÃ§in Ã¶zel saÄŸlÄ±k profilleri oluÅŸturun.</p>
          </div>
        </div>
      </div>
      ...
      <div className="panel" style={{ padding: "32px", borderRadius: "24px", marginBottom: "32px" }}>
        <div style={{ display: "flex", gap: "12px", marginBottom: "24px", padding: "6px", background: "#f8fafc", borderRadius: "16px", width: "fit-content" }}>
          ...
          >
            <Baby size={20} weight={type === 'CHILD' ? "duotone" : "regular"} /> Ã‡ocuk Profili
          </button>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div>
              <label style={{ display: "block", fontSize: "14px", fontWeight: 500, color: "#475569", marginBottom: "6px" }}>
                {type==='PREGNANCY' ? 'Profil AdÄ± (Ã–rn: Ä°lk GebeliÄŸim) *' : 'Ã‡ocuÄŸun AdÄ± *'}
              </label>
              <input name="name" required style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #cbd5e1" }} />
            </div>
            {type==='CHILD' && (
              <div>
                <label style={{ display: "block", fontSize: "14px", fontWeight: 500, color: "#475569", marginBottom: "6px" }}>YakÄ±nlÄ±k Derecesi</label>
                <input name="relationLabel" placeholder="Ã–rn: KÄ±zÄ±m" style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #cbd5e1" }} />
              </div>
            )}
            {type==='PREGNANCY' ? (
              <>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "14px", fontWeight: 500, color: "#475569", marginBottom: "6px" }}>Tahmini DoÄŸum Tarihi</label>
                  <input name="dueDate" type="date" style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #cbd5e1", color: "#475569" }} />
                </div>
              </>
            ) : (
              <div>
                <label style={{ display: "block", fontSize: "14px", fontWeight: 500, color: "#475569", marginBottom: "6px" }}>DoÄŸum Tarihi</label>
                <input name="birthDate" type="date" style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #cbd5e1", color: "#475569" }} />
              </div>
            )}
          </div>
          <div>
            <label style={{ display: "block", fontSize: "14px", fontWeight: 500, color: "#475569", marginBottom: "6px" }}>Ã–zel Notlar</label>
            <input name="notes" placeholder="Eklemek istediÄŸiniz notlar..." style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #cbd5e1" }} />
          </div>
      ...
      <div style={{ display: "grid", gap: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              ...
              <div>
                ...
                <span style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", color: "#64748b" }}>
                  ...
                  {p.type==='PREGNANCY' 
                    ? (p.dueDate ? 'Tahmini DoÄŸum: ' + new Date(p.dueDate).toLocaleDateString('tr-TR') : 'Tarih belirtilmedi')
                    : (p.birthDate ? 'DoÄŸum: ' + new Date(p.birthDate).toLocaleDateString('tr-TR') : 'DoÄŸum tarihi belirtilmedi')
                  }
                </span>
              </div>
            </div>
        ...
        {!rows.length && (
          <div style={{ padding: "40px", textAlign: "center", background: "#f8fafc", borderRadius: "16px", border: "1px dashed #cbd5e1", color: "#64748b" }}>
            HenÃ¼z oluÅŸturulmuÅŸ bir Ã¶zel profil bulunmuyor.
          </div>
        )}
      </div>
    </div>
