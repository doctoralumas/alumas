...
async function doctorPatientAccess(patientId: string, doctorId: string) {
  const [consent, share, rel] = await Promise.all([
  ...
  return Boolean(rel && ((consent?.status === 'active' && consent.scopes.includes('labs')) || share));
}
...
export async function GET(req: Request) {
  ...
  let userId = u.id; // Default: user querying their own labs

  // If a doctor is querying ANOTHER patient's labs
  if (patientIdQuery && u.role === 'DOCTOR' && u.doctorProfile) {
    if (!(await doctorPatientAccess(patientIdQuery, u.doctorProfile.id))) {
      return NextResponse.json({ error: 'Erişim yok' }, { status: 403 });
    }
    userId = patientIdQuery;
  }

  const where: any = { userId };
  if (test) where.normalizedName = norm(test);
  if (from || to) {
    where.measuredAt = {
  ...
  return NextResponse.json(await prisma.labResult.findMany({
    where,
...
export async function POST(req: Request) {
  ...
  if (!b.testName || b.value === undefined) return NextResponse.json({ error: "Test adı ve sonuç gerekli" }, { status: 400 });
  
  const numericValue = num(b.numericValue ?? b.value);
  ...
  const row = await prisma.labResult.create({
    data: {
      userId: u.id,
      testName: String(b.testName).slice(0, 120),
      normalizedName: norm(String(b.testName)).slice(0, 120),
      panel: b.panel ? String(b.panel).slice(0, 120) : null,
