...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  if(!activeConsent&&!reportShare&&!summaryShare)return <div className="page workspace"><div className="page-title"><span className="kicker">Hasta kaydı</span><h1>{patient?.name||'Hasta'}</h1><p>Bu hasta sağlık verilerine veya rapor trendlerine henüz erişim izni vermedi.</p></div><a className="secondary" href="/doctor">Uzman paneline dön</a></div>;
  ...
  return <div className="page workspace">
    ...
    {(activeConsent||reportShare)&&<><HealthGoals patientId={id}/><DoctorLabReview patientId={id}/><ImagingCenter patientId={id} doctorMode/><DiagnosticRequests patientId={id} doctorMode/><DoctorPatientNotes patientId={id}/></>}
  </div>
}
